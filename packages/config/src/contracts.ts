...
export const ENV_VAR_SPECS: EnvVarSpec[] = [
  ...
  {
    name: "FAPSHI_COLLECTION_API_USER",
    services: ["api", "worker"],
    requirement: "optional",
    secret: true,
    description: "Collection service apiuser (preferred over FAPSHI_API_USER)",
  },
  {
    name: "FAPSHI_COLLECTION_API_KEY",
    services: ["api", "worker"],
    requirement: "optional",
    secret: true,
    description: "Collection service apikey",
  },
  {
    name: "FAPSHI_PAYOUT_API_USER",
    services: ["api", "worker"],
    requirement: "strict",
    secret: true,
    description: "Payout service apiuser — distinct from collection (never mixed)",
  },
  {
    name: "FAPSHI_PAYOUT_API_KEY",
    services: ["api", "worker"],
    requirement: "strict",
    secret: true,
    description: "Payout service apikey — distinct from collection",
  },
  {
    name: "FAPSHI_PAYOUT_BASE_URL",
    services: ["api", "worker"],
    requirement: "optional",
    description: "Optional separate base URL for payout service",
  },
  {
    name: "FINANCE_STEP_UP_TOKEN",
    services: ["api"],
    requirement: "strict",
    secret: true,
    description: "Step-up / MFA token required for finance mutating commands",
  },
  {
    name: "FINANCE_PAYOUT_VELOCITY_MAX",
    services: ["api"],
    requirement: "optional",
    description: "Max payouts per actor per hour (default 20)",
  },
  {
    name: "FINANCE_FRAUD_HOLD_USER_IDS",
    services: ["api"],
    requirement: "optional",
    description: "Comma-separated user IDs blocked from finance payout/compensation",
  },
  {
