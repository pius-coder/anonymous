...
function serverHealthLabel(health?: ServerHealthState) {
  if (!health || health.status === "checking") {
    return { label: "API…", tone: "text-white/45" };
  }
  if (health.status === "down") {
    return { label: "API hors ligne", tone: "text-[--arena-danger]" };
  }
  if (health.status === "slow") {
    return {
      label: health.latencyMs ? `API ${health.latencyMs}ms` : "API lente",
      tone: "text-[--arena-gold]",
    };
  }
  return {
    label: health.latencyMs ? `API ${health.latencyMs}ms` : "API ok",
...
export function LiveRoomShell({
  ...
  const localPlayer =
    players.find((player) => player.userId === currentUserId) ?? activePlayers[0] ?? players[0];
  ...
  const requests =
    externalSocialRequests ??
    (fallbackRequests.length > 0 ? fallbackRequests : buildSocialRequests(players, groups));
  ...
  const health = serverHealthLabel(serverHealth);
    ...
    <main className="fixed inset-0 z-50 h-dvh w-screen overflow-hidden bg-[--arena-ink] text-white">
      ...
      <header className="safe-area-top pointer-events-none absolute inset-x-0 top-0 z-20 flex items-start justify-between gap-2 px-3 md:px-5">
        <div className="premium-toolbar pointer-events-auto flex min-w-0 items-center gap-3 px-3 py-2.5">
          ...
          <div className="min-w-0">
            <p className="truncate font-head text-sm font-black uppercase tracking-wide">
              Social Survivor
            </p>
            <div className="mt-0.5 flex items-center gap-2 text-[10px] uppercase tracking-[0.16em] text-white/45">
              <span
                className={cn(
                  "status-dot",
                  status === "connected" ? "text-[--arena-green]" : "text-[--arena-gold]",
                )}
              />
            ...
            <div
              className={cn(
                "mt-0.5 flex items-center gap-1.5 text-[10px] uppercase tracking-[0.12em]",
                health.tone,
              )}
            >
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="premium-toolbar pointer-events-auto max-w-[54vw] px-3 py-2.5 text-right md:max-w-sm">
            ...
            <p className="truncate font-head text-sm font-black uppercase md:text-base">
              {currentGameName || "Carte sociale"}
            </p>
          </div>
          <button
            type="button"
      ...
      {localPlayer && !showGameSurface && (
        <div className="pointer-events-none absolute bottom-24 left-3 z-10 hidden max-w-sm md:block">
          <button
            type="button"
            onClick={() => openPlayer(localPlayer.userId)}
            className="premium-floating pointer-events-auto flex w-full items-center gap-3 p-3 text-left"
          >
            <Avatar className="size-12 border border-white/18">
              {localPlayer.avatarUrl ? (
                <AvatarImage src={localPlayer.avatarUrl} alt={localPlayer.displayName} />
              ) : null}
              <AvatarFallback className="bg-gradient-to-br from-primary to-[#7356ff] font-head">
                {initials(localPlayer)}
              </AvatarFallback>
            </Avatar>
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                <p className="truncate font-head text-sm font-black uppercase">
              ...
              <p className="mt-1 truncate text-xs text-white/48">
                {currentGroup?.name ?? "Aucun groupe"} · Centre de la carte
              </p>
            </div>
          ...
          >
            <div className="absolute inset-x-0 top-0 z-10 flex items-center justify-between border-b border-white/10 bg-black/25 px-3 py-2 backdrop-blur-md">
              <div className="flex min-w-0 items-center gap-2">
                ...
                <p className="truncate font-head text-xs font-black uppercase">
                  {currentGameName || "Mini-jeu"}
                </p>
              </div>
              <button
                type="button"
                onClick={() => setGameExpanded(false)}
                className="flex items-center gap-1 rounded-lg px-2 py-1 text-[10px] uppercase text-white/48 hover:bg-white/7 hover:text-white"
              >
        ...
        {activePanel !== "NONE" && (
          ...
          >
            <button
              ...
              className="absolute right-3 top-3 z-20 grid size-8 place-items-center rounded-xl border border-white/10 bg-black/25 text-white/55 hover:text-white"
      ...
      <div className="safe-area-bottom pointer-events-none absolute inset-x-0 bottom-0 z-40 px-2 md:px-4">
        <nav
          className="premium-toolbar pointer-events-auto mx-auto flex max-w-xl items-center justify-between gap-1 p-1.5"
          aria-label="Navigation sociale du lobby"
        >
          <button
            type="button"
            onClick={() => {
            ...
            className={cn(
              "flex min-w-0 flex-1 flex-col items-center gap-1 rounded-xl px-2 py-2 text-[9px] font-bold uppercase transition",
              activePanel === "NONE"
          ...
          {panelActions.map(({ panel, label, icon: Icon }) => (
            <button
              ...
              className={cn(
                "relative flex min-w-0 flex-1 flex-col items-center gap-1 rounded-xl px-2 py-2 text-[9px] font-bold uppercase transition",
                activePanel === panel
      ...
      <div className="pointer-events-none absolute bottom-[5.7rem] right-3 z-10 hidden flex-col gap-2 lg:flex">
        <button
          type="button"
          onClick={() => onPing("here")}
          className="premium-toolbar pointer-events-auto grid size-11 place-items-center text-white/60 hover:text-white"
          aria-label="Placer un signal ici"
        >
          <Target className="size-4" />
        </button>
        <div className="premium-toolbar flex items-center gap-2 px-3 py-2 text-xs">
          <Radio className="size-3 text-[--arena-green]" />
          <span className="font-head font-black">
      ...
      <AnimatePresence>
        {showElimination && (
          ...
          >
            ...
            >
              <p className="font-head text-5xl font-black uppercase text-[--arena-danger]">
                Éliminé
              </p>
              <p className="mt-3 text-sm leading-relaxed text-white/58">
                La partie continue. Tu peux observer les joueurs sans recevoir d’informations
                privées supplémentaires.
              </p>
              <Button
                className="mt-5 w-full"
                onClick={() => {
                  setEliminationDismissed(true);
