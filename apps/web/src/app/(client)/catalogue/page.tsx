...
async function getSessions(page = 1, limit = 20, filter = "all") {
  const apiBase = process.env.API_URL || "http://localhost:3001";
  const params = new URLSearchParams({ page: String(page), limit: String(limit), filter });
  const res = await fetch(`${apiBase}/v1/public/sessions?${params.toString()}`, {
    cache: "no-store",
  });
...
const FILTERS = [
  ...
  { key: "capacity", label: "Capacité restante" },
] as const;
...
export default async function CataloguePage({
  ...
  const filter = FILTERS.some((f) => f.key === filterParam) ? (filterParam as string) : "all";
  const result = await getSessions(page, 20, filter);
