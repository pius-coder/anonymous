...
import {
  ...
  adminStartRoundBodySchema,
  authorizeLiveRoundStart,
  livePauseBodySchema,
  pauseLiveSession,
  resumeLiveSession,
  serializeLiveSessionState,
} from "../../live/live.js";
import { publishLiveCommand } from "../../live/liveCommands.js";
...
adminLive.post(
  "/live/:sessionId/rounds/start",
  requireAuth,
  requireRole("ADMIN", "SUPER_ADMIN"),
  zValidator("param", adminLiveSessionParamsSchema, validationHook),
  zValidator("json", adminStartRoundBodySchema, validationHook),
  async (c) => {
    const user = c.get("user");
    const { sessionId } = c.req.valid("param");
    const body = c.req.valid("json");
    const result = await authorizeLiveRoundStart({
      adminUserId: user.id,
      sessionId,
      roundNum: body.roundNum,
    ...
    await publishLiveCommand(result.command);
